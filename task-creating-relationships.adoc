...
Continuous Sync:: After the initial data sync, Cloud Sync listens for changes on the source S3 bucket and continuously syncs any changes to the target as they occur. There's no need to rescan the source at scheduled intervals.
+
This setting is available only when creating the sync relationship and when you sync from an S3 bucket to S3, Google Cloud Storage, Azure Blob storage, StorageGRID, or IBM Storage.
+
...
* Filter by size will be active only on copy events (not on delete events).
