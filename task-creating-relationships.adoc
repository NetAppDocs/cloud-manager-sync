...
Continuous Sync:: After the initial data sync, Cloud Sync listens for changes on the source S3 bucket and continuously syncs any changes to the target as they occur. There's no need to rescan the source at scheduled intervals. This setting is available only when you create the sync relationship and when an S3 bucket is the source.
+
If you enable this setting, it affects other features as follows:
+
* The following settings are reverted to their default values: Schedule, Sync Timeout, Recently Modified Files, and Date Modified.
